            ...
            setChangedResourcesData((prev) => {
              ...
              const mergeChanges = (existing: any[], incoming: any[]) => {
                ...
                incoming.forEach((incomingItem) => {
                  ...
                  if (existingIndex >= 0) {
                    // 如果已存在，更新记录（保留最新的trigger_resource_datas，确保trigger_resource_action字段不丢失）
                    ...
                    merged[existingIndex] = {
                      ...merged[existingIndex],
                      ...incomingItem,
                      trigger_resource_datas: mergedTriggerData,
                    };
                  } else {
                    // 如果不存在，添加新记录
      ...
      onFinish={async (values) => {
        try {
          ...
          Object.entries(changedResourcesData).forEach(([modelKey, changes]) => {
            ...
            // 已移除cleanTriggerData函数，直接在各个操作中处理触发器数据
            ...
            if (changes.created && changes.created.length > 0) {
              modelResources.push(
                ...changes.created.map((item) => {
                  ...
                  // 如果有trigger_resource_datas，也需要清理其中的id和index字段，并设置trigger_resource_action为create
                  if (
                    resourceItem.trigger_resource_datas &&
                    Array.isArray(resourceItem.trigger_resource_datas)
                  ) {
                    resourceItem.trigger_resource_datas = resourceItem.trigger_resource_datas.map(
                      (trigger: any) => {
                        // eslint-disable-next-line @typescript-eslint/no-unused-vars
                        const { id, index, ...cleanTrigger } = trigger;
                        // 为新创建的监控项的触发器设置action为create
            ...
            if (changes.updated && changes.updated.length > 0) {
              modelResources.push(
                ...changes.updated.map((item) => {
                  ...
                  // 如果有trigger_resource_datas，也需要清理其中的id和index字段，并保持原有的trigger_resource_action字段
                  if (
                    resourceItem.trigger_resource_datas &&
                    Array.isArray(resourceItem.trigger_resource_datas)
                  ) {
                    resourceItem.trigger_resource_datas = resourceItem.trigger_resource_datas.map(
                      (trigger: any) => {
                        // eslint-disable-next-line @typescript-eslint/no-unused-vars
                        const { id, index, ...cleanTrigger } = trigger;
                        // 保持触发器原有的action状态，如果没有则默认为update
                        if (!cleanTrigger.trigger_resource_action) {
                          cleanTrigger.trigger_resource_action = 'update';
                        }
                        return cleanTrigger;
                      },
                    );
            ...
            if (changes.deleted && changes.deleted.length > 0) {
              modelResources.push(
                ...changes.deleted.map((item) => {
                  ...
                  // 如果有trigger_resource_datas，也需要清理其中的id和index字段，删除的监控项触发器也标记为delete
                  if (
                    resourceItem.trigger_resource_datas &&
                    Array.isArray(resourceItem.trigger_resource_datas)
                  ) {
                    resourceItem.trigger_resource_datas = resourceItem.trigger_resource_datas.map(
                      (trigger: any) => {
                        // eslint-disable-next-line @typescript-eslint/no-unused-vars
                        const { id, index, ...cleanTrigger } = trigger;
                        // 为删除的监控项的触发器设置action为delete
                        cleanTrigger.trigger_resource_action = 'delete';
                        return cleanTrigger;
                      },
                    );
                  }
          ...
          // 调试：检查触发器数据中的trigger_resource_action字段
          Object.entries(resourcesData).forEach(([modelId, resources]) => {
            console.log(`模型 ${modelId} 的资源数据:`, resources);
            resources.forEach((resource, index) => {
              if (
                resource.trigger_resource_datas &&
                Array.isArray(resource.trigger_resource_datas)
              ) {
                console.log(`  资源 ${index} 的触发器数据:`, resource.trigger_resource_datas);
                resource.trigger_resource_datas.forEach((trigger: any, triggerIndex: number) => {
                  console.log(`    触发器 ${triggerIndex}:`, {
                    trigger_resource_action: trigger.trigger_resource_action,
                    hasField: 'trigger_resource_action' in trigger,
                  });
                });
              }
            });
